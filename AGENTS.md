...
- **Static Analysis**: z3dk's `oracle_analyzer.py --check-hooks --find-mx` validates M/X register state at hook entry points (e.g., JumpTableLocal requires X=16-bit). Run via build script or manually from `~/src/hobby/z3dk/scripts/`.
