...
- Dialogue changes are **UNBLOCKED**. For expanded IDs `$18D+`, update
  `Data/dialogue/expanded_messages.json` through Yaze source sync, regenerate
  `Core/Generated/expanded_messages.asm`, rebuild with
  `Scripts/Build/build_rom.sh 168`, then reopen or reload
  `Roms/oos168x.sfc` for inspection. Direct editor or CLI writes to ASM-owned
  bank `$2F` are not durable, and the patched ROM is test-only.
