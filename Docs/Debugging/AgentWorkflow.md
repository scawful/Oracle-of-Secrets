...
- Client: `python3 Scripts/Mesen2/mesen2_client.py` (auto-attaches only if a single socket exists; otherwise set `MESEN2_SOCKET_PATH` or use `--socket`/`--instance`).
- Build: `./Scripts/Build/build_rom.sh 168` (or version from ROM name).
...
python3 Scripts/Mesen2/mesen2_client.py watch --profile overworld
```
...
- Use `./Scripts/Mesen2/mesen2_launch_instance.sh --headless --instance <name> --source ci --owner agent`.
...
### Modules (coarse)
- File: `Config/module_flags.asm` (generated by `Scripts/Build/set_module_flags.py`)
```
python3 Scripts/Build/set_module_flags.py --profile all --disable dungeon
python3 Scripts/Build/set_module_flags.py --profile all   # restore
```

### Features (fine)
- File: `Config/feature_flags.asm` (generated by `Scripts/Build/set_feature_flags.py`)
```
python3 Scripts/Build/set_feature_flags.py --list
python3 Scripts/Build/set_feature_flags.py --disable water_gate_hooks,water_gate_overlay_redirect
python3 Scripts/Build/set_feature_flags.py --profile defaults   # restore to Util/macros.asm defaults
```

Notes:
- `Scripts/Build/build_rom.sh` regenerates `hooks.json` automatically when flag files change, so z3dk analysis matches the built ROM.
...
Build script note:
- `Scripts/Build/build_rom.sh` runs the analyzer automatically. By default it is **non-fatal**; set `OOS_ANALYSIS_FATAL=1` to block builds on analyzer errors.

## Sanity checks (rare but handy)
```
ls -ld /Applications/Mesen2\ OOS.app
./Scripts/Mesen2/mesen2_sanity_check.sh --instance <name>   # checks socket + ROM alignment
```

## Optional helpers
- `python3 Scripts/Mesen2/mesen2_client.py` or `Scripts/Mesen2/mesen2_client_lib/bridge.py` for scripted socket access.
