...
- **Regression guardrails**: use feature/module flags plus the autonomous debugger to catch hardlocks early.
  - Smoke suite: `bash Scripts/Validate/run_regression_tests.sh smoke --no-moe --fail-fast`
...
2. Load in Mesen2 and confirm the named instance with
...
- Verify the build process (`Scripts/Build/build_rom.sh` and manual Asar flow).
