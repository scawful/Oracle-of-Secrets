...
*** Latest Handoff (Lore + Infra Planning)
    - See: =Docs/Planning/Status/handoff_lore_infra_planning_2026-01-24.md=
...
*** ACTIVE [#B] Feature Flags + Tooling Alignment
    ...
    - *Tasks:*
      - [X] Standardize feature overrides: =Config/feature_flags.asm= + =Scripts/Build/set_feature_flags.py=
...
*** DONE [#A] Resolve ZSOW vs. Lost Woods Conflict
    ...
    - *Regression:* Minor camera/coordinate desync when returning West (0x28). Documented in [[file:Docs/Debugging/Issues/LostWoods_Transition_Desync.md][LostWoods_Transition_Desync.md]]. Low priority.
