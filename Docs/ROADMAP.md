...
### Feature flags
- Canonical override file: `Config/feature_flags.asm` (generated via `Scripts/Build/set_feature_flags.py`).
- Use feature flags for risky `org` patches so hooks can be isolated without backing out commits.
- When flags change, `Scripts/Build/build_rom.sh` regenerates `hooks.json` automatically (so z3dk analysis matches the built ROM).
