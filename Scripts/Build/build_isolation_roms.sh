...
for mod in "${MODULES[@]}"; do
  ...
  # Set flags
  python3 Scripts/Build/set_module_flags.py --profile all --disable "$mod"

  # Build (skip tests + analysis)
  set +e
  SKIP_TESTS=1 OOS_ANALYSIS_FATAL=0 ./Scripts/Build/build_rom.sh 168 --skip-tests --no-symbols 2>&1 | tail -5
  rc=$?
  set -e
