...
Usage:
  Scripts/Build/dev_loop.sh [version] [options]
...
Examples:
  Scripts/Build/dev_loop.sh 168 --z3asm --mesen-sync --reload --validate
  Scripts/Build/dev_loop.sh --version 168 --asar /usr/local/bin/asar --skip-tests
  Scripts/Build/dev_loop.sh 168 --yaze-restart --yaze-sync
EOF
}
...
env "${env_args[@]}" "${ROOT_DIR}/Scripts/Build/build_rom.sh" "${build_args[@]}"
