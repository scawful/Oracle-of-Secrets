...
def parse_room_list(raw: str) -> List[int]:
    return [int(token.strip(), 0) for token in raw.split(",") if token.strip()]
...
def write_asm(
    ...
    lines.append("; Generated by Scripts/Generate/generate_water_fill_table.py")
...
def main() -> int:
    ...
    parser.add_argument(
        "--require-rooms",
        default="",
        help="Fail unless every comma-separated room has marker tiles.",
    )
    ...
    rooms = sorted(marker_offsets)
    required_rooms = parse_room_list(args.require_rooms)
    invalid_required = [
        room_id for room_id in required_rooms if not 0 <= room_id <= 0xFF
    ]
    if invalid_required:
        raise SystemExit(
            "Invalid room in --require-rooms: "
            + ", ".join(f"{room_id:#x}" for room_id in invalid_required)
        )
    missing_rooms = [
        room_id for room_id in required_rooms if room_id not in marker_offsets
    ]
    if missing_rooms:
        raise SystemExit(
            "Required water-fill rooms have no marker tiles: "
            + ", ".join(f"0x{room_id:02X}" for room_id in missing_rooms)
        )
