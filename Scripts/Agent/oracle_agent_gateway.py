...
def action_run_test_suite(args: dict[str, Any]) -> dict[str, Any]:
    oos_root = _resolve_oos_root()
    if not oos_root:
        return {"ok": False, "error": "Oracle-of-Secrets root not found"}
    script = oos_root / "scripts" / "run_regression_tests.sh"
    ...
    suite = "regression"
    if isinstance(args, dict):
        raw_suite = str(args.get("suite", "regression")).strip().lower()
        if raw_suite in {"smoke", "regression", "full"}:
            suite = raw_suite
    return _spawn(["bash", str(script), suite], cwd=oos_root)
...
ACTIONS: dict[str, dict[str, Any]] = {
    ...
    "run_smoke_tests": {
        ...
        "description": "Run manifest smoke suite",
    },
    "run_test_suite": {
        "fn": action_run_test_suite,
        "description": "Run configured regression suite",
    },
    "health": {
        "fn": action_health,
